...
.defines(function() {
	Win = ig.Game.extend({
		...
		init: function() {
			...
			// set achievements
			( new Clay.Achievement( { id: 'rp5' } ) ).award();
			if (this.score > 1000)
		...
		draw: function() {
			this.parent();
			this.fontLarge.draw('CONGRATULATIONS', 100, 70, ig.Font.ALIGN.CENTER);
			this.font.draw('You managed to get', 100, 90, ig.Font.ALIGN.CENTER);
			this.fontLarge.draw(this.score, 100, 100, ig.Font.ALIGN.CENTER);
			this.font.draw('points.', 100, 115, ig.Font.ALIGN.CENTER);
			this.font.draw('You are the greatest.', 100, 140, ig.Font.ALIGN.CENTER);
			if( ig.ua.mobile ) {
